...
public class DefaultCaptchaProvider implements ICaptchaProvider {
    ...
    public void init(ICaptcha owner) throws Exception {
        ...
        //
        __cage = new Cage(new Painter(owner.getModuleCfg().getWidth(), owner.getModuleCfg().getHeight(), owner.getModuleCfg().getBackground(), owner.getModuleCfg().getQuality(), _effect, _random),
                _fonts, _foregrounds, owner.getModuleCfg().getFormat(), owner.getModuleCfg().getCompressRatio(), new RandomTokenGenerator(_random, owner.getModuleCfg().getTokenLengthMin()), _random);
    }
